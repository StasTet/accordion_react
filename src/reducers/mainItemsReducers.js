...
export default function reducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case LOAD_DATA_REQUESTED:
      return { loading: true, data: action.data, errors: action.errors };

    case LOAD_DATA_OK:
      return {
        loading: false,
        data: action.data.map(item => _.assign({}, item, {id: _.uniqueId(), visible: false})),
        errors: action.errors
      }

    case LOAD_DATA_FAIL:
      return { loading: false, data: action.data, errors: action.errors };
